...
export default function TVPage() {
  ...
  const served = state.entries.filter((e) => e.status === 'served').slice(-4).reverse()
        ...
        <div className="w-80 flex flex-col gap-4">
          ...
          <div className="flex flex-col gap-2 overflow-y-auto flex-1">
            ...
            {waiting.map((e, i) => {
              ...
              const ownMin = calcMinutes(e.services)
              const totalWait = waitBefore + ownMin
              ...
              return (
                ...
                >
                  <span className="text-zinc-600 w-5 text-right text-sm shrink-0">{i + 1}.</span>
                  <span className="text-2xl font-black text-amber-400 w-12 shrink-0">{e.ticket}</span>
          ...
          {served.length > 0 && (
            <>
              <h2 className="text-zinc-700 uppercase tracking-widest text-xs font-semibold">
                Atendidos
              </h2>
              <div className="flex flex-col gap-1.5">
                {served.map((e) => (
                  <div
                    key={e.id}
                    className="flex items-center gap-3 bg-zinc-900/50 rounded-xl px-4 py-2 opacity-40"
                  >
                    <span className="text-lg font-bold text-zinc-600 w-12">{e.ticket}</span>
                    <span className="text-zinc-500 text-sm truncate">{e.name}</span>
                  </div>
                ))}
              </div>
            </>
